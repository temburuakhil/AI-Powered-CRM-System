...
// Google Sheets URL - Registration Details Sheet (2nd tab)
const SHEET_ID = "1y7LyjjyKRMX4XSTjPA3lu4hM9gMl27e2QElXiG4FZp8";
const REGISTRATION_GID = "661516192"; // Registration Details sheet GID
...
const RegistrationDetails = () => {
  ...
  const [rowStatus, setRowStatus] = useState<Map<number, 'approved' | 'rejected'>>(new Map());
  ...
  const handleApprove = async (rowIndex: number, rowData: Record<string, any>) => {
    ...
    try {
      ...
      const img = new Image();
      img.src = `${webhookUrl}?${params.toString()}&t=${Date.now()}`;
      ...
      // Set the row status to approved
      setRowStatus(prev => new Map(prev).set(rowIndex, 'approved'));
  ...
  const handleReject = async (rowIndex: number, rowData: Record<string, any>) => {
    ...
    try {
      ...
      // Set the row status to rejected
      setRowStatus(prev => new Map(prev).set(rowIndex, 'rejected'));
